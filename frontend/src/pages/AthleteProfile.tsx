...
function HistoryBlock({ athleteId }: { athleteId: string }) {
  const { data: history = [], isLoading } = useQuery<WorkoutHistoryItem[]>({
    queryKey: ['history', athleteId],
    queryFn: () => athletesApi.getHistory(athleteId, 10),
  })

  return (
    <Paper p="md" withBorder radius="md">
      <Title order={4} mb="sm">
        Последние 10 тренировок — план / факт
      </Title>

      {isLoading && <Loader size="sm" />}

      {!isLoading && history.length === 0 && (
        <Text size="sm" c="dimmed">Тренировок ещё нет</Text>
      )}
        ...
        <Table withTableBorder withColumnBorders fz="sm">
          <Table.Thead>
            <Table.Tr>
              ...
              <Table.Th ta="right">Plan TSS</Table.Th>
              <Table.Th ta="right">Fact TSS</Table.Th>
          ...
          <Table.Tbody>
            {history.map((w) => {
              ...
              return (
                <Table.Tr key={w.id} style={{ background: rowBg }}>
                  ...
                  <Table.Td>
                    <Badge
                      size="xs"
                      color={w.status === 'completed' ? 'green' : 'blue'}
                      variant="light"
                    >
                      {w.status === 'completed' ? 'Завершено' : 'В плане'}
                    </Badge>
                  </Table.Td>

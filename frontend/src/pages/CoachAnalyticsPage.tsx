...
function getGranularity(type: PeriodType, from: string, to: string): 'daily' | 'weekly' {
...
function VolumePeriodSelector(props: PeriodSelectorProps) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      {/* Custom date pickers shown below when type === 'custom' */}
      {type === 'custom' && (
...
function VolumeSection({ data, granularity }: { data: VolumeResponse; granularity: 'daily' | 'weekly' }) {
  ...
  return (
    <Stack gap="md">
      ...
      {/* Stacked bar: daily or weekly depending on granularity */}
      {chartFlat.length > 0 && (
        <Paper withBorder p="md" radius="md">
...
function AthleteAnalytics({ athleteId }: { athleteId: string }) {
...
export default function CoachAnalyticsPage() {
  ...
  const athleteSelectData = useMemo(
    () => athletes.map(a => ({ value: a.athlete_id, label: a.full_name })),
    [athletes]
  )
    ...
    <Container size="xl" py="xl">
      ...
      {!selectedAthleteId ? (
        <Paper withBorder p={48} radius="md" style={{ textAlign:'center' }}>
          ...
          <Text size="sm" c="dimmed">Выберите атлета из списка выше для просмотра аналитики</Text>
        </Paper>
      ) : (
        <>
          <Group mb="lg" gap="xs">
